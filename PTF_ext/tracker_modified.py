...
def pseudonym_change_events(dataframe, pseudonyms):
    ...
    for i in tqdm(pseudonyms.tolist()):  #show a smart progress meter of the loop
        ...
        if len(pseudonyms_events) > 1:
            dataframe.loc[pseudonyms_events.iloc[0].name, 'event'] = 'e' #.name= numero riga
...
def local_change(dataframe, pseudonyms, beacon_interval, results, dimensions=False):
    ...
    for p in tqdm(pseudonyms.tolist()):
        ...
        #time_interval = possible_match['ts_rx'].between(last_seen_time, last_seen_time + beacon_interval + time_tolerance)
        ...
        if not possible_match.empty:
            ...
            for k in range(len(possible_match)):
                ...
                if near(last_seen['speed'] * (current['ts_rx'] - last_seen_time), current['distance'], 2):
...
def filter_dataframe(dataframe, pseudonyms):
    ...
    for v in vehicles.tolist():
        ...
        if len(vehicles_pseudonyms) == 1: #doesn't exist next pseudonym
            to_remove_pseudonyms = np.append(to_remove_pseudonyms, np.where(pseudonyms == vehicles_pseudonyms))
    """
    
    for v in vehicles.tolist():
        vehicles_pseudonyms = np.array(dataframe.loc[dataframe['realID'] == v]['pseudonym'].unique())
        #remove always the last pseudonym of a vehicle, it always exists
...
def analyze(path, freq, dimensions):
    ...
    dataframe, pseudonyms = mean_pseudonyms_change(path)  #df concat of all rsu file
...
def main(base_folder, freq, policy, dimensions):
    ...
    results_file = 'results_track_mod.csv'
    ...
    with open(results_file, 'a') as f:
        if head:
            f.write('fq,pc,prec,recall,f1_score,dimensions\n')
        f.write(f'{freq}, {policy}, {precision}, {recall}, {f1_score}, {dimensions}\n')
